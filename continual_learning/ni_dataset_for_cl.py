...
""" Modified from https://github.com/yizhongw/Tk-Instruct/blob/main/src/ni_dataset.py """
...
class NIConfig(datasets.BuilderConfig):
    def __init__(self, *args, task_dir=None, 
        max_num_instances_per_task=None, 
        max_num_instances_per_eval_task=None,
        task_split_file_name=None, 
        load_official_test=True,
        **kwargs):
        ...
        self.task_split_file_name=task_split_file_name
        self.load_official_test=load_official_test
...
class NaturalInstructions(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        if self.config.load_official_test:
            return [
                datasets.SplitGenerator(
                    name=datasets.Split.TRAIN,
                    gen_kwargs={
                        "path": os.path.join(split_dir, f"{self.config.task_split_file_name}.txt"), 
                        "task_dir": task_dir, 
                ...
                datasets.SplitGenerator(
                    ...
                    gen_kwargs={
                        "path": os.path.join(split_dir, "test_tasks.txt"), # this is the official split of test set
                        "task_dir": task_dir, 
        ...
        with open(path, encoding="utf-8") as split_f:
            for line in split_f:
                ...
                with open(task_path, encoding="utf-8") as task_f:
                    ...
                    if subset == "official_test" or subset == "test":
                        if max_num_instances_per_task is not None and max_num_instances_per_task >= 0:
                            ...
                            print(f"subset: {subset}, len: {len(instances)}, max_num_instances_per_task: {max_num_instances_per_task}")
                    else:
                        instances = all_instances

                    # # for 'train', we read all instances and will perform train/dev/split later
                    # # add condition `subset == 'train'` to make sure that the test instances are not shuffled
                    # # so that test instances are always the `all_instances[:max_num_instances_per_task]`
                    # if subset == 'train':
                    #     if max_num_instances_per_task is not None and max_num_instances_per_task >= 0:
