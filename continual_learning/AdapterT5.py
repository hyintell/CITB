...
class T5AdapterStack(T5Stack):
    def add_adapters(self,bottleneck_size=100,adapter_num=40):
        self.adapter_blocks = nn.ModuleList([MixAdapter(self.config,bottleneck_size,adapter_num) for _ in range(self.config.num_layers)])
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value, adapter) in enumerate(zip(self.block, past_key_values, self.adapter_blocks)):
            ...
            hidden_states = adapter(layer_outputs[0], task_id=task_id)
...
class AdapterT5(T5ForConditionalGeneration):
    ...
    def __init__(self, config: T5Config):
        ...
        self.encoder = T5AdapterStack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5AdapterStack(decoder_config, self.shared)
    ...
    def add_adapters(self, bottleneck_size=100, adapter_num=40):
        self.encoder.add_adapters(bottleneck_size, adapter_num)
        self.decoder.add_adapters(bottleneck_size, adapter_num)
    ...
    # def clinit_adapter(self, task_id):
    #     if task_id > 0:
    #         for layern in range(self.config.num_layers):
    #             self.encoder.adapter_blocks[layern].mixadapter[task_id] = copy.deepcopy(self.encoder.adapter_blocks[layern].mixadapter[task_id-1])
    #             self.decoder.adapter_blocks[layern].mixadapter[task_id] = copy.deepcopy(self.decoder.adapter_blocks[layern].mixadapter[task_id-1])
