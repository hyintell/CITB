...
do 
    ...
    output_dir="output/continual_instruction_tuning/stream=${task_split_file_name}/CL=${cl_method}/instances${max_num_instances_per_task}_epoch${num_train_epochs}_bs${per_device_train_batch_size}_lr${learning_rate}_seed${seed}"
    run_name="CL=${cl_method}_stream=${task_split_file_name}_seed=${seed}"
    echo $output_dir
    echo $run_name
