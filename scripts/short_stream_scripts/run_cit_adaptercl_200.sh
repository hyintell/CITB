...
data_dir_for_task_order="data/CIT_data/task_orders/stream=${task_split_file_name}"


# for ((i=1; i<=3; i=i+1))
# for ((i=1; i<=1; i=i+1))

for order in 1 2 3 1
# for order in 1
do 
    seed=$(shuf -i 10-999 -n 1)
    echo Running using seed=$seed

    output_dir="output/continual_instruction_tuning/stream=${task_split_file_name}/CL=${cl_method}_bottleneck_size${bottleneck_size}/bottleneck_size${bottleneck_size}_epoch${num_train_epochs}_bs${per_device_train_batch_size}_lr${learning_rate}_order${order}_seed${seed}"
